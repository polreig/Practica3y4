...
pew_tidy<-gather(pew_religion,key="income",value="freq",-religion)

income_levels<-c("<$10k", "$10-20k", "$20-30k", "$30-40k", "$40-50k", 
                   "$50-75k", "$75-100k", "$100-150k", ">150k")

pew_tidy$income<-factor(pew_tidy$income, levels = income_levels)
...
```{r}
load("data/billboard.Rdata")
billboard_tidy<-raw%>%pivot_longer(cols=starts_with("wk"),names_to="week", values_to="rank", names_prefix="wk")%>%mutate(week=as.integer(week))
...
tb_tidy<-raw%>%
  gather(key="demographic",value="cases",-country,-year,na.rm=TRUE)%>%
  separate(demographic,into=c("sex","age"),sep=1)
...
weather_tidy<-raw%>%
  pivot_longer(cols=starts_with("d"),names_to="day",values_to="value",names_prefix="d",values_drop_na=TRUE)%>%
...
planes.f<-planes%>%
  ...
  mutate(manufacturer=ifelse(str_detect(str_to_lower(manufacturer),".*airbus"),'AIRBUS COMP',manufacturer))
...
```{r }
# Para flights: eliminar time_hour y variables que terminan con "time"
flights <- flights %>%
  select(-time_hour, -ends_with("time"))

# Para planes: eliminar speed y type
planes <- planes %>%
  select(-speed, -type)
```
...
```{r }
# Crear la variable Full_Date y eliminar variables originales
flights <- flights %>%
  mutate(
    Full_Date = paste(day, month, year.x, sep = "-")  # Formato d-m-Y
  ) %>%
  select(-year.x, -month, -day)  # Eliminar las variables originales
